...
fn run_json(arguments: &[&str]) -> Value {
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("removed in v0.4"));
    serde_json::from_slice(&output.stdout).unwrap()
}

#[test]
fn versioned_api_exposes_embedded_content_without_a_legacy_warning() {
    let request = temporary_json(
        "content-info",
        &serde_json::json!({
            "operation": "content_info"
        }),
    );
    let output = binary()
        .args(["api", "--input", request.to_str().unwrap()])
        .output()
        .unwrap();
    ...
    assert_eq!(response["value"]["schema_version"], 1);
    assert_eq!(
        response["value"]["package"]["package_id"],
        "spire-codex-stable-v0.107.1"
    );
