...
DEFAULT_USER_AGENT = "sls2-combat-catalog-crawler/0.2"
ENTITY_ENDPOINTS = (
    "cards",
    "characters",
    "relics",
...
REQUIRED_FIELDS = {
    ...
    "characters": frozenset(("id", "name", "max_energy", "starting_hp")),
    "relics": frozenset(("id", "name", "description", "rarity")),
    "potions": frozenset(("id", "name", "description", "rarity")),
    "monsters": frozenset(("id", "name", "type", "moves")),
    "powers": frozenset(("id", "name", "type", "stack_type")),
    "encounters": frozenset(("id", "name", "monsters", "room_type")),
    "ascensions": frozenset(("id", "name", "level", "description")),
}
...
def fetch_spire_codex_snapshot(
    ...
    *,
    ...
    channel_query = "" if channel == "stable" else "?channel=beta"
    entity_requests = tuple(
        (endpoint, f"{endpoint}{channel_query}") for endpoint in ENTITY_ENDPOINTS
    )
    requests = (
        (("changelogs", "changelogs"), *entity_requests)
        if channel == "stable"
        else (
